...
const selectClass = 'mt-1 block w-full rounded-md border-0 bg-surface py-2 px-3 text-main ring-1 ring-inset ring-line dark:bg-surface-muted dark:text-main';
const optionClass = 'bg-surface text-main dark:bg-surface-muted dark:text-main';
            ...
            {showNewForm ? (
              <form onSubmit={createLead}>
                ...
                <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <label className="block"><span className="text-sm font-medium text-main">State</span><select value={form.state} onChange={e => setForm({ ...form, state: e.target.value })} className={selectClass}><option value="" className={optionClass}>Select</option>{STATES.map(state => <option key={state} value={state} className={optionClass}>{state}</option>)}</select></label>
                  {isAdmin && <label className="block"><span className="text-sm font-medium text-main">Assign Rep</span><select value={form.assigned_rep_id} onChange={e => setForm({ ...form, assigned_rep_id: e.target.value })} className={selectClass}><option value="" className={optionClass}>Unassigned</option>{salesReps.map(rep => <option key={rep.id} value={rep.id} className={optionClass}>{rep.name}</option>)}</select></label>}
              ...
              <div>
                ...
                <div className="p-6 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <label className="block"><span className="text-sm font-medium text-main">Status</span><select value={selectedLead.status} disabled={selectedLead.status === 'converted'} onChange={e => void updateLead({ status: e.target.value as LeadStatus })} className={selectClass}>{selectedLead.status === 'converted' && <option value="converted" className={optionClass}>Converted</option>}{STATUS_OPTIONS.map(status => <option key={status.value} value={status.value} className={optionClass}>{status.label}</option>)}</select></label>
                    {isAdmin && <label className="block"><span className="text-sm font-medium text-main">Assign Rep</span><select value={selectedLead.assigned_rep_id || ''} onChange={e => void updateLead({ assigned_rep_id: e.target.value || null })} className={selectClass}><option value="" className={optionClass}>Unassigned</option>{salesReps.map(rep => <option key={rep.id} value={rep.id} className={optionClass}>{rep.name}</option>)}</select></label>}
                  </div>
                  <div className="flex flex-wrap gap-2">
