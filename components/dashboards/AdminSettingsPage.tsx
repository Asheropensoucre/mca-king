...
export const AdminSettingsPage: React.FC<AdminSettingsPageProps> = ({ currentUser, onSalesRepCreated }) => {
  ...
  if (!isCurrentUserAdmin) return <Card className="p-6 text-sm text-danger">Admin settings are admin-only.</Card>;

  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div>
        <h2 className="text-2xl font-black text-main ">Admin Settings</h2>
        <p className="mt-1 text-sm text-muted">Manage broker-shop user accounts. Normal users can only change their own password.</p>
      </div>
          ...
          <div className="p-6">
            <form onSubmit={runSearch} className="mb-5 flex flex-wrap items-end gap-3">
              <label className="text-xs font-black uppercase tracking-wider text-secondary">
                Role
                <select value={role} onChange={event => setRole(event.target.value as UserRole | '')} className="mt-1 block rounded-md border px-3 py-2 text-sm -muted">
                  {roleOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
                </select>
              </label>
              <label className="text-xs font-black uppercase tracking-wider text-secondary">
                Status
                <select value={status} onChange={event => setStatus(event.target.value)} className="mt-1 block rounded-md border px-3 py-2 text-sm -muted">
              ...
              <label className="min-w-64 flex-1 text-xs font-black uppercase tracking-wider text-secondary">
                Search
                <input value={search} onChange={event => setSearch(event.target.value)} className="mt-1 block w-full rounded-md border px-3 py-2 text-sm -muted" placeholder="Name or email" />
              </label>
                ...
                <table className="min-w-full border-separate border-spacing-y-2">
                  ...
                  <tbody>
                    {filteredUsers.map(user => {
                      ...
                      return (
                        <tr key={user.id} className="bg-surface-muted/50">
                          <td className="px-3 py-3 text-sm font-black text-main ">{user.full_name ?? 'N/A'}</td>
                          <td className="px-3 py-3 text-sm text-muted">{user.email}</td>
                          <td className="px-3 py-3 text-sm">
                            <select value={user.role} onChange={event => updateRole(user, event.target.value as UserRole)} disabled={user.closed_at !== null || user.id === currentUser.id} className="rounded-md border px-2 py-1 text-xs font-bold -muted">
                          ...
                          <td className="px-3 py-3 text-sm text-muted">{formatDate(user.last_login_at)}</td>
                    ...
                    {filteredUsers.length === 0 && <tr><td colSpan={7} className="px-3 py-8 text-center text-sm text-muted">No users found.</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
      ...
      {tab === 'create_rep' && (
        <Card>
          <form onSubmit={createSalesRep} className="p-6">
            <h3 className="text-lg font-black text-main ">Create Sales Rep</h3>
            <p className="mt-1 text-sm text-muted">This creates an internal broker-shop sales rep login.</p>
      ...
      {selectedUser && action && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <Card className="w-full max-w-md">
            <div className="p-6">
              <h3 className="text-lg font-black text-main ">{actionTitle(action)}</h3>
              <p className="mt-1 text-sm text-muted">User: {selectedUser.email}</p>
              ...
              {action === 'close' && <p className="mt-3 rounded-md bg-danger/10 p-3 text-sm font-semibold text-danger dark:bg-danger/20 dark:text-danger">This cannot be undone from the user side. CRM history will be preserved.</p>}
            </div>
...
const StatusBadge: React.FC<{ status: 'Closed' | 'Disabled' | 'Active' }> = ({ status }) => {
  const classes = status === 'Active'
    ...
    : status === 'Disabled'
      ? 'bg-warning/15 text-warning dark:bg-warning/25 dark:text-warning'
      : 'bg-danger/15 text-danger dark:bg-danger/25 dark:text-danger';
