...
export const AdminFinanceView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {message && <p className="rounded-md bg-theme-teal/10 px-3 py-2 text-sm text-theme-teal">{message}</p>}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <Card><div className="p-4"><p className="text-xs font-black uppercase text-slate-500">Funded Volume</p><p className="mt-2 text-2xl font-black text-theme-maroon dark:text-theme-yellow">{money(totals.funded)}</p></div></Card>
        <Card><div className="p-4"><p className="text-xs font-black uppercase text-slate-500">Revenue Expected</p><p className="mt-2 text-2xl font-black text-theme-maroon dark:text-theme-yellow">{money(totals.revenueExpected)}</p></div></Card>
        <Card><div className="p-4"><p className="text-xs font-black uppercase text-slate-500">Revenue Received</p><p className="mt-2 text-2xl font-black text-theme-maroon dark:text-theme-yellow">{money(totals.revenueReceived)}</p></div></Card>
        <Card><div className="p-4"><p className="text-xs font-black uppercase text-slate-500">Rep Commission Liability</p><p className="mt-2 text-2xl font-black text-theme-maroon dark:text-theme-yellow">{money(totals.commissionsUnpaid)}</p></div></Card>
      </div>

      <Card>
        <div className="p-6">
          <h3 className="text-lg font-black text-theme-maroon dark:text-theme-yellow">Funded Deals</h3>
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
              <thead className="bg-slate-950/90"><tr className="text-left text-xs font-black uppercase tracking-wider text-theme-yellow"><th>Merchant</th><th>Lender/Funder</th><th>Amount</th><th>Factor</th><th>Funded</th></tr></thead>
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {fundings.length > 0 ? fundings.map(item => <tr key={item.id}><td className="px-4 py-3 text-sm font-bold">{item.merchant_name ?? 'N/A'}</td><td className="px-4 py-3 text-sm">{item.lender_name ?? 'N/A'}</td><td className="px-4 py-3 text-sm">{money(item.funded_amount)}</td><td className="px-4 py-3 text-sm">{item.factor_rate ?? 'N/A'}</td><td className="px-4 py-3 text-sm">{date(item.funded_at)}</td></tr>) : <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-slate-500">No funded deals yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      <Card>
        <div className="p-6">
          <h3 className="text-lg font-black text-theme-maroon dark:text-theme-yellow">Broker Revenue</h3>
          <div className="mt-4 space-y-3">
            {revenue.length > 0 ? revenue.map(item => (
              <div key={item.id} className="grid grid-cols-1 gap-3 rounded-lg border border-slate-200 p-4 dark:border-slate-700 md:grid-cols-5 md:items-center">
                <div className="md:col-span-2"><p className="font-bold">{item.merchant_name ?? 'N/A'}</p><p className="text-sm text-slate-500">{item.lender_name ?? 'N/A'}</p></div>
                <p className="text-sm font-black">{money(item.amount)}</p>
                <p className="text-sm text-slate-500">{item.revenue_type}</p>
            ...
            )) : <p className="text-sm text-slate-500">No broker revenue records yet.</p>}
          </div>
        </div>
      </Card>
      ...
      <Card>
        <div className="p-6">
          <h3 className="text-lg font-black text-theme-maroon dark:text-theme-yellow">Sales Rep Commissions</h3>
          <div className="mt-4 space-y-3">
            {commissions.length > 0 ? commissions.map(item => (
              <div key={item.id} className="grid grid-cols-1 gap-3 rounded-lg border border-slate-200 p-4 dark:border-slate-700 md:grid-cols-5 md:items-center">
                <div className="md:col-span-2"><p className="font-bold">{item.sales_rep_name ?? 'Unassigned Rep'}</p><p className="text-sm text-slate-500">{item.merchant_name ?? 'N/A'}</p></div>
                <p className="text-sm font-black">{money(item.amount)}</p>
                <p className="text-sm text-slate-500">{item.basis_type}</p>
            ...
            )) : <p className="text-sm text-slate-500">No sales rep commission records yet.</p>}
          </div>
        </div>
      </Card>
    </div>
