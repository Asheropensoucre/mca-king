...
export const ManualEmailModal: React.FC<Props> = ({ entityType, entityId, defaultTo, onClose, onSent }) => {
  ...
  return (
    <ResponsiveModal className="max-w-2xl" ariaLabel="Send Email">
      <div className="space-y-4 p-5 sm:p-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-black text-main ">Send Email</h3>
          <button onClick={onClose} className="text-sm font-bold text-secondary">Close</button>
        </div>
        ...
        <p className="text-xs font-semibold text-muted">Email sends through Resend only. Suppression/do-not-contact is checked server-side before sending.</p>
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-end"><PrimaryButton label="Cancel" size="small" variant="danger" onClick={onClose} /><PrimaryButton label={sending ? 'Sending…' : 'Send Email'} size="small" disabled={sending || !subject || !body} onClick={() => void send()} /></div>
      </div>
    </ResponsiveModal>
  );
};
