...
const PipelineCardContent: React.FC<{ merchant: FormData; lenders: LenderInfo[]; compact?: boolean; cardNumber?: number }> = ({ merchant, lenders, compact = false, cardNumber }) => {
  ...
  return (
    <div className="w-full shrink-0 rounded-xl border-2 border-line bg-surface p-5 text-main shadow-[6px_6px_0_rgb(var(--color-border-strong))] transition-all hover:border-secondary dark:shadow-[6px_6px_0_rgb(var(--color-accent))]">
      <div>
        <div className="mb-2 flex flex-wrap items-center gap-2">
          {typeof cardNumber === 'number' && <span className="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-black text-on-secondary">Deal {cardNumber}</span>}
          <span className="rounded-full bg-accent px-2 py-0.5 text-[10px] font-black text-on-accent">{merchant.status}</span>
        </div>
        <p className="text-[10px] font-black uppercase tracking-wider text-secondary">Business Name</p>
        <div className="truncate text-base font-black text-main" title={getBusinessName(merchant)}>
          {getBusinessName(merchant)}
        </div>
        <p className="mt-1 truncate text-xs font-semibold text-muted" title={getPrimaryOwnerName(merchant)}>Owner: {getPrimaryOwnerName(merchant)}</p>
      </div>
      <div className="mt-2 space-y-2">
        <div className="border-b border-line pb-1.5 text-xs text-muted">
          Requested Amount: <span className="font-semibold text-main">{money(merchant.requestedAmount)}</span>
        </div>
        <div className="border-b border-line pb-1.5 text-xs text-muted">
          Primary Phone: <span className="font-medium text-main">{getPrimaryPhone(merchant)}</span>
        </div>
        <div className="truncate border-b border-line pb-1.5 text-xs text-muted" title={getPrimaryEmail(merchant)}>
          Primary Email: <span className="font-medium text-main">{getPrimaryEmail(merchant)}</span>
        </div>
        <div className="border-b border-line pb-1.5 text-xs text-muted">
          Business State / TZ: <span className="font-medium text-main">{getStateFromAddress(merchant.businessInfo.address)}</span>
        </div>
        <div className="border-b border-line pb-1.5 text-xs text-muted">
          Matched Lenders / Offers: <span className="font-medium text-main">{matchedCount} matched / {offersCount} offer{offersCount === 1 ? '' : 's'}</span>
        </div>
        {matchedLenderNames && (
          <div className="truncate border-b border-line pb-1.5 text-xs text-muted" title={matchedLenderNames}>
            Sent to Lenders: <span className="font-medium text-main">{matchedLenderNames}</span>
          </div>
        )}
        {acceptedOffer && (
          <div className="truncate border-b border-line pb-1.5 text-xs text-success ">
            Accepted: {acceptedOffer.lenderName} / {money(acceptedOffer.amount)}
          </div>
        )}
        <div className="text-xs text-muted">
          Contract Signed: <span className="font-medium text-main">{signedLabel}</span>
        </div>
      </div>
    </div>
  );
...
const SortablePipelineCard: React.FC<{
  ...
  return (
    <button
      ...
      className="mb-4 w-full touch-none cursor-grab rounded-lg text-left last:mb-0 focus:outline-none focus:ring-2 focus:ring-secondary active:cursor-grabbing"
    >
...
const DroppableColumn: React.FC<{
  ...
  return (
    <div className={`flex h-full w-[330px] shrink-0 flex-col rounded-2xl border border-line bg-surface-muted shadow-sm ${themeClasses.border}`}>
      <button
        type="button"
        onClick={() => onOpenStep(column.status)}
        className="sticky top-0 z-10 flex flex-col gap-2 rounded-t-2xl border-b border-line bg-surface-muted p-5 text-left transition hover:border-secondary hover:bg-surface-strong focus:outline-none focus:ring-2 focus:ring-secondary"
        aria-label={`Open expanded view for ${column.status}`}
      >
        <span className="text-[10px] font-black uppercase tracking-wider text-secondary">Kamba Step {idx + 1}</span>
...
const ExpandedPipelineStep: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[68] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Expanded Kamba step">
      <div className="flex max-h-[92vh] w-full max-w-7xl flex-col overflow-hidden rounded-2xl border-2 border-line-strong bg-surface text-main shadow-[10px_10px_0_rgb(var(--color-border-strong))] dark:shadow-[10px_10px_0_rgb(var(--color-accent))]">
        <div className="flex flex-col gap-4 border-b-2 border-line bg-surface-muted p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-wider text-secondary">Kamba Pipeline Step {currentIndex + 1} of {total}</p>
            <h2 className="mt-1 text-3xl font-black text-main">Step {currentIndex + 1}: {column.status}</h2>
            <p className="mt-1 text-sm font-semibold text-muted">This enlarged step view contains {column.cards.length} named deal card{column.cards.length === 1 ? '' : 's'} currently in this pipeline stage.</p>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-6">
          <div className={`mb-5 rounded-xl border-2 bg-surface-muted p-5 -strong ${themeClasses?.border ?? 'border-line'}`}>
            <h3 className={`text-lg font-black ${themeClasses?.title ?? 'text-main'}`}>Step Overview: {column.status}</h3>
            <p className="mt-2 text-sm font-semibold text-muted">Every card below is labeled by deal number, business name, owner, amount, contact info, and lender/offer status. Click a named deal card to open the full deal view.</p>
          </div>

          {column.cards.length > 0 ? (
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
              {column.cards.map((merchant, cardIdx) => (
                <button key={merchant.id} type="button" onClick={() => onOpenCard(merchant)} className="text-left focus:outline-none focus:ring-2 focus:ring-secondary rounded-xl" aria-label={`Open deal ${cardIdx + 1}: ${getBusinessName(merchant)}`}>
                  <div className="mb-2 rounded-lg bg-primary px-3 py-2 text-sm font-black text-on-primary  dark:text-on-accent">
            ...
            <div className="rounded-xl border border-line bg-surface-muted p-8 text-center font-bold text-muted -strong ">
...
const ExpandedPipelineCard: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Expanded Kamba card">
      <div className="flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border-2 border-line-strong bg-surface text-main shadow-[10px_10px_0_rgb(var(--color-border-strong))] dark:shadow-[10px_10px_0_rgb(var(--color-accent))]">
        <div className="flex flex-col gap-4 border-b-2 border-line bg-surface-muted p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-wider text-secondary">Kamba Deal Card {currentIndex + 1} of {total}</p>
            <h2 className="mt-1 text-3xl font-black text-main">{getBusinessName(merchant)}</h2>
            <p className="mt-1 text-sm font-semibold text-muted">Current Step: {merchant.status} • Owner: {getPrimaryOwnerName(merchant)}</p>
          </div>
          <div className="flex flex-wrap gap-2">
        ...
        <div className="grid min-h-0 flex-1 gap-6 overflow-y-auto p-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-5">
            ...
            <div className="rounded-xl border border-line bg-surface-muted p-5 -strong">
              <h3 className="text-lg font-black text-main">Business Snapshot</h3>
              <dl className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div><dt className="text-xs font-black uppercase text-secondary">Requested Amount</dt><dd className="font-bold text-main ">{money(merchant.requestedAmount)}</dd></div>
                <div><dt className="text-xs font-black uppercase text-secondary">Monthly Revenue</dt><dd className="font-bold text-main ">{money(merchant.businessInfo.monthlyRevenue)}</dd></div>
                <div><dt className="text-xs font-black uppercase text-secondary">Industry</dt><dd className="font-bold text-main ">{merchant.businessInfo.industryType || 'N/A'}</dd></div>
                <div><dt className="text-xs font-black uppercase text-secondary">State / TZ</dt><dd className="font-bold text-main ">{getStateFromAddress(merchant.businessInfo.address)}</dd></div>
                <div><dt className="text-xs font-black uppercase text-secondary">Entity Type</dt><dd className="font-bold text-main ">{merchant.businessInfo.entityType || 'N/A'}</dd></div>
                <div><dt className="text-xs font-black uppercase text-secondary">Recent NSFs</dt><dd className="font-bold text-main ">{merchant.businessInfo.recentNSFs || 'N/A'}</dd></div>
              </dl>
            </div>
          </div>
          ...
          <div className="space-y-5">
            <div className="rounded-xl border border-line bg-surface-muted p-5 -strong">
              <h3 className="text-lg font-black text-main">Primary Contact</h3>
              <dl className="mt-4 space-y-3">
                <div><dt className="text-xs font-black uppercase text-secondary">Name</dt><dd className="font-bold text-main ">{primaryOwner?.name || 'N/A'}</dd></div>
                <div><dt className="text-xs font-black uppercase text-secondary">Phone</dt><dd className="font-bold text-main ">{getPrimaryPhone(merchant)}</dd></div>
                <div><dt className="text-xs font-black uppercase text-secondary">Email</dt><dd className="break-all font-bold text-main ">{getPrimaryEmail(merchant)}</dd></div>
                <div><dt className="text-xs font-black uppercase text-secondary">Credit Score</dt><dd className="font-bold text-main ">{primaryOwner?.creditScore || 'N/A'}</dd></div>
              </dl>
            </div>

            <div className="rounded-xl border border-line bg-surface-muted p-5 -strong">
              <h3 className="text-lg font-black text-main">Pipeline Stage</h3>
              <div className="mt-4 flex flex-wrap gap-2">
                <span className={`rounded-full px-3 py-1 text-xs font-black ${statusTheme?.badge ?? 'bg-surface-strong text-main'}`}>{merchant.status}</span>
                <span className="rounded-full bg-secondary px-3 py-1 text-xs font-black text-on-secondary">{merchant.offers?.length || 0} Offer{(merchant.offers?.length || 0) === 1 ? '' : 's'}</span>
                <span className="rounded-full bg-accent px-3 py-1 text-xs font-black text-on-accent">{merchant.matchedLenderIds?.length || 0} Lender{(merchant.matchedLenderIds?.length || 0) === 1 ? '' : 's'}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="flex h-[calc(100vh-2rem)] min-h-[720px] flex-col lg:h-[calc(100vh-4rem)]">
      <div className="mb-5 flex flex-col gap-4 rounded-2xl border border-line bg-surface px-6 py-5 shadow-sm   xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="flex flex-wrap gap-3 text-sm">
          <span className="rounded-full bg-surface-muted px-4 py-2 font-semibold text-muted -muted ">{normalizedMerchants.length} total deal{normalizedMerchants.length === 1 ? '' : 's'}</span>
          <span className="rounded-full bg-accent px-4 py-2 font-semibold text-on-accent">12 pipeline steps</span>
        </div>
      </div>
