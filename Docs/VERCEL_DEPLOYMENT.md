...
This guide explains how to deploy MCA King to Vercel safely on the Hobby plan. MCA King is a Vite + React app with server-side API route handlers. Local development uses a Vite middleware bridge, while production on Vercel uses **one bundled root-level catch-all serverless function** in the `api/` directory.
...
Vercel static frontend build from dist/
  ↓
Single bundled Vercel serverless function: api/index.js
  ↓
...
Vercel does not run the Vite dev middleware. Production API calls are handled by one root-level bundled Vercel function:

```txt
api/index.js
```

This file is generated by `bun run build:api` from:

```txt
src/server/vercel-entry.ts
```

The source entry imports `src/server/api.ts`, and Bun bundles that central router plus its route dependencies into `api/index.js`. This avoids Node ESM extensionless-import crashes in Vercel production while keeping the deployment compatible with the Vercel Hobby plan's 12-function limit. Do not hand-edit `api/index.js`; update `src/server/vercel-entry.ts` or `src/server/api.ts`, then rerun `bun run build:api`.
...
api/index.js               # Generated bundled catch-all Vercel serverless function
scripts/build-api.ts        # Bun bundling script that generates api/index.js
src/server/vercel-entry.ts  # Source Vercel adapter/wrapper for the bundled function
...
All production API routes are handled by this single bundled Vercel function:

```txt
api/index.js
```

The `vercel.json` rewrite forwards all `/api/:path*` requests to the `/api` function (`api/index.js`) with the original path captured as a `path` query parameter. The bundled function reconstructs the original `/api/...` path before calling the bundled central router from `src/server/api.ts`, which currently routes:
...
The source catch-all adapter in `src/server/vercel-entry.ts` disables Vercel body parsing:
...
Normal JSON routes still work because the bundled catch-all function reads the raw request stream and forwards it as a Web `Request` to the central API router.
...
`bun run build` runs both parts of the deployment build:

```txt
bun run build:api  # bundles src/server/vercel-entry.ts into api/index.js
bun run build:web  # builds the Vite frontend into dist/
```
...
Check that the generated root API function exists:

```txt
api/index.js
```

If it is missing locally, run:
...
The project should have only one API function entry file:
...
Do **not** add a new Vercel file for every route. The deployment uses the single bundled `api/index.js` function plus a `/api/:path*` → `/api?path=:path*` rewrite to stay compatible with the Vercel Hobby plan. Update route logic in `src/routes` / `src/server/api.ts`, then run `bun run build:api` so the generated Vercel function stays current.
